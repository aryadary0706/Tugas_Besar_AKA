...
// Fungsi Yang dipakai pada Iteratif Dan Rekursif
void ClassificationRole(Players &Class_position, int &j, Players DataPlayer, string position, int overall, int N) {
    j = 0; // Inisialisasi j ke 0
    for (int i = 0; i < N; i++) {
        if (DataPlayer.role[i] == position && DataPlayer.Overall[i] <= overall && DataPlayer.Overall[i] >= overall - 1) {
...
output SequentialSearchIterative(Players Dataplayer, string position, int Overall, int value, int N) {
    ...
    // Loop pencarian dengan peningkatan toleransi
        while (!pemain_ketemu && i < N) {
            if ((Dataplayer.value[i] >= (value - (value*0.1))  && Dataplayer.value[i] <= value)) {
    ...
    return CalonPemain; // Kembalikan hasil pencarian
}

output binarysearchplayerIterative(Players Dataplayer, string position, int Overall, int value, int N) {
    output CalonPemain;
    CalonPemain = {-1, "", -1}; // Inisialisasi hasil pencarian
    int low = 0;
    int high = N - 1;

    while (low <= high) {
        int mid = low + (high - low) / 2; // Menghindari overflow pada (low + high) / 2

        // Cek apakah pemain di indeks tengah memenuhi kriteria
        if (Dataplayer.value[mid] > value - (value * 0.1) && Dataplayer.value[mid] <= value) {
            CalonPemain.Id = Dataplayer.ID[mid];
            CalonPemain.name = Dataplayer.nama[mid];
            CalonPemain.Harga = Dataplayer.value[mid];
            return CalonPemain; // Pemain ditemukan
        }

        // Jika nilai pemain di tengah kurang dari batas bawah nilai, pindah ke sisi kanan
        if (Dataplayer.value[mid] < value - (value * 0.1)) {
            low = mid + 1;
        } else {
            // Jika nilai pemain di tengah lebih besar dari nilai target, pindah ke sisi kiri
            high = mid - 1;
        }
    }

    // Jika tidak ada pemain yang memenuhi kriteria
    return CalonPemain;
}
...
output SequentialSearchRecursive(Players Dataplayer, string position, int Overall, int value, int N, int i) {
    ...
    // Basis rekursi: Jika seluruh elemen telah diperiksa
    if (i >= N) {
        return CalonPemain;
    }

    // Periksa apakah pemain saat ini memenuhi kriteria
    if (Dataplayer.value[i] >= value - (value*0.1)  && Dataplayer.value[i] <= value) {
        CalonPemain.Id = Dataplayer.ID[i];
        CalonPemain.name = Dataplayer.nama[i];
        CalonPemain.Harga = Dataplayer.value[i];
        return CalonPemain; // Kembalikan pemain yang ditemukan
    }
    cout<<Dataplayer.nama[i]<<endl;
    // Panggilan rekursif dengan indeks berikutnya
...
output binarysearchplayerRecursive(Players Dataplayer, string position, int Overall, int value, int N, int low, int high) {
    ...
    cout<<Dataplayer.nama[mid]<<endl;
    if (Dataplayer.value[mid] > value - (value*0.1)  && Dataplayer.value[mid] <= value) {
        // Player found, populate the output
        CalonPemain.Id = Dataplayer.ID[mid];
        CalonPemain.name = Dataplayer.nama[mid];
        CalonPemain.Harga = Dataplayer.value[mid];
        return CalonPemain;
    }
    // If the middle player's value is less than the target value, search the right half
    if (Dataplayer.value[mid] <= value-(value*0.1)) {
